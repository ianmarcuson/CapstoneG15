# py correr_intradia_replicas_farmacia_tardia_24h.py --replicas 30 --workers-intradia 1 --replica-workers 1 --overwrite


"""
correr_intradia_replicas_farmacia_tardia_24h.py

Ejecuta modelo_intradia_farmacia_anticipada_24h.py para multiples outputs del modelo interdia,
usando la variante que prefiere preparar farmacia anticipada lo mas tarde posible,
por ejemplo:
...
Uso tipico desde la carpeta:
    CapstoneG15/Modelo INTRAdia V2 Farmacia Anticipada 24h Farmacia Tardia

    py correr_intradia_replicas_farmacia_tardia_24h.py --replicas 30 --workers-intradia 1

Supuestos por defecto:
    - Este script esta en CapstoneG15/Modelo INTRAdia V2 Farmacia Anticipada 24h Farmacia Tardia
    - modelo_intradia_farmacia_anticipada_24h.py esta en la misma carpeta
    - Los outputs interdia FA estan en ../Modelo Interdia Farmacia Anticipada H450/resultados_farmacia_anticipada_h450
    - El patron esperado es solution_interday_farmacia_anticipada_h450-i.xlsx
...
def run_intradia_for_replica(
    ...
    input_pattern: str = "solution_interday_farmacia_anticipada_h450-{replica}.xlsx",
) -> IntradiaRunResult:
    input_solution = interdia_dir / input_pattern.format(replica=replica)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ejecuta modelo_intradia_farmacia_anticipada_24h.py para todas las replicas solution_interday-i.xlsx usando farmacia tardia."
    )
    parser.add_argument("--replicas", type=int, default=30, help="Cantidad de replicas a correr.")
    parser.add_argument("--replica-start", type=int, default=1, help="Primera replica a correr.")
    parser.add_argument(
        "--replica-end",
        type=int,
        default=None,
        help="Ultima replica a correr. Si se omite, usa --replicas.",
    )
    parser.add_argument(
        "--interdia-dir",
        type=str,
        default="../Modelo Interdia Farmacia Anticipada H450/resultados_farmacia_anticipada_h450",
        help="Carpeta con outputs interdia por replica.",
    )
    parser.add_argument(
        "--input-pattern",
        type=str,
        default="solution_interday_farmacia_anticipada_h450-{replica}.xlsx",
        help="Patron de nombre del output interdia. Usa {replica} como placeholder.",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
        default="resultados_intradia_farmacia_tardia_30_replicas",
    ...
    parser.add_argument(
        "--model-file",
        type=str,
        default="modelo_intradia_farmacia_anticipada_24h.py",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Workers internos del modelo intradia. En 24h se recomienda 1 por dependencia secuencial entre dias.",
    )
    parser.add_argument(
        "--replica-workers",
        type=int,
...
def main() -> None:
    ...
    replica_start = int(args.replica_start)
    replica_end = int(args.replica_end) if args.replica_end is not None else int(args.replicas)
    if replica_start <= 0:
        raise ValueError("--replica-start debe ser >= 1")
    if replica_end < replica_start:
        raise ValueError("--replica-end debe ser >= --replica-start")
    ...
    _safe_print(f"Replicas:            {replica_start}..{replica_end}")
    ...
    missing_inputs = [
        interdia_dir / args.input_pattern.format(replica=i)
        for i in range(replica_start, replica_end + 1)
        if not (interdia_dir / args.input_pattern.format(replica=i)).exists()
    ]
    if missing_inputs:
