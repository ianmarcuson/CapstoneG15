...
def resolve_existing_path(path: Optional[str], candidates: List[Path], label: str) -> str:
    """Resuelve rutas relativas de forma portable.

    Prioridad:
    1) Ruta explícita entregada por CLI.
    2) Candidatos relativos al directorio del script/proyecto.
    """
    tried: List[Path] = []
...
def load_interday_assignments(solution_path: Optional[str], base_data_path: Optional[str]) -> Tuple[pd.DataFrame, Dict]:
    ...
    print(
        ...
        f"días={assignments['day'].nunique()}, rango_días=[{assignments['day'].min()}..{assignments['day'].max()}]"
    )
...
def parse_args():
    ...
    parser.add_argument("--solution", default=None, help="Excel de salida del modelo interday. Default: solution_interday.xlsx en la misma carpeta del script")
    parser.add_argument("--base-data", default=None, help="Excel con parámetros base. Default: ../Data Inicial/Data G15.xlsx")
