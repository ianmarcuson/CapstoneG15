...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stall-seconds", type=int, default=60, help="Segundos sin mejora de gap antes de cortar por callback.")
    parser.add_argument("--stall-min-runtime", type=int, default=120, help="Runtime minimo antes de permitir corte por estancamiento.")
    parser.add_argument("--stall-min-gap-improvement", type=float, default=1e-4, help="Mejora minima de gap para reiniciar el contador de estancamiento.")
    parser.add_argument("--disable-gap-stall", action="store_true", help="Desactiva el callback de corte por estancamiento del gap.")
...
def main() -> None:
    ...
    if args.stall_min_gap_improvement < 0:
        raise ValueError("--stall-min-gap-improvement debe ser >= 0")
    ...
    if not args.disable_gap_stall:
        print(
            f"Stall callback:   {args.stall_seconds}s sin mejora | "
            f"min_runtime={args.stall_min_runtime}s | "
            f"min_improvement={args.stall_min_gap_improvement}"
        )
    print("=" * 72)
